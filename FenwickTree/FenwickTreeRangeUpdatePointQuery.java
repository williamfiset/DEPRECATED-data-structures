public class FenwickTreeRangeUpdatePointQuery {
	...
	public FenwickTreeRangeUpdatePointQuery(long[] values) {
		...
		cloneTree=new long[values.length + 1];
		
		long[] feniTree=new long[values.length+1];
		long val=0;
		int j=0;
		for(int i=1;i<feniTree.length;i++){
			val=values[i-1];
			j=i;
			while(j<feniTree.length){
				feniTree[j]+=val;
				j+=lsb(j);
			}
		}
	...
	public long getPoint(int index, long[] val) {
		int sum1 = 0, sum2=0;
